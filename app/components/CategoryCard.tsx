...
export default function CategoryCard({ name, passedCount, totalCount, isCompleted, onSelect }: CategoryCardProps) {
    ...
    <button onClick={onSelect} className={`w-full flex items-center justify-between p-5 rounded-xl border transition-all transform active:scale-[0.98] card-hover ${
      ...
      <div className="flex-1 text-left">
        <div className="flex items-center gap-3">
          {icon}
          <span className="font-black text-gray-800 dark:text-white text-lg">{clean}</span>
          {isCompleted && <FaCheckCircle className="text-green-500" size={16} />}
        </div>
